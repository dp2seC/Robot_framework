...
Library  DataDriver  ../../EX_Files/Queries/R2M_Query.xlsx   sheet_name=SRC_IMED
Suite Setup  Run Keywords  Connect to  RODS  
...                   AND  Connect to  IMEDIDATA 
...                   AND  Connect to  MEDS 
...                   AND  Get StudyID and STU_UUID
Suite Teardown  Run Keywords  DB Disconnect  RODS 
...                      AND  DB Disconnect  IMEDIDATA
...                      AND  DB Disconnect  MEDS
Test Teardown  Test Case Error Handling
Test Template  Test Scenarios

# Command: robot -d Results/R2M_Results Tests/03_RODS_TO_MEDS/03_SRC_IMED.robot
# use for debug: robot -d Results/R2M_Results -L DEBUG Tests/03_RODS_TO_MEDS/03_SRC_IMED.robot
# use to run parallel: pabot -d Results/R2M_Results Tests/03_RODS_TO_MEDS
# use when no tags: robot -d Results/R2M_Results -r NONE -l NONE -o NONE  Tests/03_RODS_TO_MEDS/03_SRC_IMED.robot
# use for tag: robot -d Results/R2M_Results -r NONE -l NONE -o NONE -v Default_Tag:Smoke  Tests/03_RODS_TO_MEDS/03_SRC_IMED.robot
